...
def execute(sql, con, retry=True, cur=None, params=None):
    ...
    try:
        ...
        print('Exception on execute: %s' % (e,))
        print('Query: %s' % (sql,))
        try:
            con.rollback()
        except Exception as e:  # pragma: no cover
            print('Exception on rollback: %s' % (e,))
...
def _safe_columns(cols):
    return [s.replace(' ', '_').replace('-', '_').strip() for s in cols]
...
def _write_sqlite(frame, table, names, cur):
    bracketed_names = ['[' + column + ']' for column in _safe_columns(names)]
...
def _write_postgresql(frame, table, names, cur):
    ...
    wildcards = ','.join([r'%s'] * len(names))
    insert_query = 'INSERT INTO %s (%s) VALUES (%s)' % (
        table, col_names.lower(), wildcards)
    data = [tuple(x) for x in frame.values]
    #print insert_query % data[0]
    cur.executemany(insert_query, data)
...
def table_exists(name, con, flavor):
    flavor_map = {
        ...
        'postgresql' : "SELECT * FROM pg_catalog.pg_tables where tablename = '%s'" % name.split('.')[-1]}
...
def get_schema(frame, name, flavor, keys=None):
    ...
    # Replace spaces in DataFrame column names with _.
    safe_columns = [s.replace(' ', '_').replace('-', '_').strip() for s in frame.dtypes.index]
